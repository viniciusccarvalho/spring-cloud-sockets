 ...
 */
public class RequestStreamSocketHandler extends AbstractReactiveSocketMethodHandler {
	@Override
	public Mono<RSocket> invoke(MethodHandler methodHandler) {
		return Mono.just(new AbstractRSocket() {
			@Override
			public Flux<Payload> requestChannel(Publisher<Payload> payloads) {
				return (Flux<Payload>) methodHandler.invoke(payloads);
			}
		});
	}
	...
	public boolean accepts(ExchangeMode exchangeMode) {
		return exchangeMode == ExchangeMode.REQUEST_STREAM;
	}
}
